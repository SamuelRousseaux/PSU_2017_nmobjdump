...
char *return_arch(Elf64_Ehdr *eh)
{
	int arch_len = 8;
...
void print_dump_bis_64(unsigned int i, unsigned int *j, unsigned char *stru,
			char *str)
{
	char tmp;
...
void print_dump_64(Elf64_Ehdr *eh, Elf64_Shdr sh)
{
	int add = sh.sh_addr;
	...
	for (unsigned int i = 0 ; i < sh.sh_size ; (void)i) {
		...
		if (!(i % 4) && i % 16 && i < sh.sh_size)
			...
			print_dump_bis_64(i, &j, str, (char *)str);
		else {
			if (!(i % 16)) {
				add += 16;
				print_dump_bis_64(i, &j, str, (char *)str);
...
void print_objd_64(Elf64_Ehdr *eh, Elf64_Shdr *sh, char *strtab)
{
	int is_ok = 0;
	char *sec_name;
	...
	for (int i = 0 ; i < eh->e_shnum ; i++) {
		...
		if (is_ok) {
			...
			print_dump_64(eh, sh[i]);
		}
	}
}

void objdump_64bits(void *data, char *name)
{
	...
	printf("architecture: %s, ", return_arch(eh));
	if (eh->e_type < ET_NUM) {
		printf("flags 0x%08x:\n", ftypes[eh->e_type]);
		printf("%s\n", flags[eh->e_type]);
	}
	printf("start address 0x%016lx\n\n", eh->e_entry);
	print_objd_64(eh, sh, strtab);
}
